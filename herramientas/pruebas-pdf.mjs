...
import {
  ...
  planificarPaginas, crearPdfStickers,
...
comprobar('capacidad de una hoja', STICKERS_POR_PAGINA, 204);
...
comprobar('dos eventos no comparten la misma hoja',
  dosEventos.map(p => p.codigo), ['A01', 'A02']);
...
comprobar('el documento contiene dos objetos Page',
  (texto.match(/\/Type \/Page\b/g) || []).length, 2);

const cajas = [...texto.matchAll(/\/MediaBox \[0 0 ([\d.]+) ([\d.]+)\]/g)];
comprobar('cada pagina declara su MediaBox', cajas.length, 2);
...
comprobar('hay un flujo de contenido por pagina', flujos.length, 2);
